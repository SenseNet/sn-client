...
export const DocumentViewerSettingsContext = React.createContext<DocumentViewerSettings>(
  new DocumentViewerSettings({
    canEditDocument: async () => false,
    canHideRedaction: async () => false,
    canHideWatermark: async () => false,
    commentActions: {
      addPreviewComment: async (_doc, c) => ({ ...c } as any),
...
export const RepositoryViewerSettings: React.FC = ({ children }) => {
  const repo = useRepository()

  return (
    <DocumentViewerSettingsContext.Provider
      value={
        new DocumentViewerSettings({
