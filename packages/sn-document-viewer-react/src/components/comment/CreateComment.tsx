...
export type CreateCommentLocalization = Pick<
  ...
  'commentInputPlaceholder' | 'addComment' | 'submit' | 'inputRequiredError' | 'markerRequiredError'
>
...
export function CreateComment(props: CreateCommentProps) {
  ...
  const validate = () => {
    ...
    if (!props.draftCommentMarker) {
      // TODO(Zoli): Show tooltip at iconbutton
  ...
  return (
    <form onSubmit={handleSubmit}>
      <FormControl error={hasError()}>
        <InputLabel htmlFor="comment-input">{localization.commentInputPlaceholder}</InputLabel>
        <Input
          id="comment-input"
          type="text"
          value={value}
          ...
          endAdornment={
            <InputAdornment position="end">
              <IconButton aria-label="Toggle" onClick={props.handlePlaceMarkerClick}>
                <SvgIcon
                  viewBox="0 0 100 125"
                  style={props.isPlacingMarker ? { transform: 'rotate(90deg)', color: 'blue' } : undefined}>
