 ...
 */
describe('ToggleWatermarkWidget component', () => {
  ...
  it('Click on toggle should toggle the watermark', () => {
    useTestContext(ctx => {
      ...
      expect(ctx.store.getState().sensenetDocumentViewer.viewer.showWatermark).not.toBe(originalValue)

      button.props.onClick()
      expect(ctx.store.getState().sensenetDocumentViewer.viewer.showWatermark).toBe(originalValue)
    })
  })
})
