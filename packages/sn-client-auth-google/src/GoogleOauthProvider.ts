...
import { ILoginResponse, IOauthProvider } from "@sensenet/authentication-jwt";
...
export class GoogleOauthProvider implements IOauthProvider {
    ...
    private async getTokenFromPrompt(loginReqUrl: string): Promise<string> {
        return new Promise<string>(async (resolve, reject) => {
            this.popup = window.open(loginReqUrl, "_blank", "toolbar=no,scrollbars=no,resizable=no,top=200,left=300,width=400,height=400", true);
