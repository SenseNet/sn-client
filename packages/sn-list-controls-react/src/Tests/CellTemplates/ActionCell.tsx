...
export const actionCellTests: Mocha.Suite = describe('ActionCell component', () => {
    it('Should render without crashing', () => {
        const component = renderer.create(<ActionsCell actions={[]} openActionMenu={() => undefined} content={{ Id: 1, Path: '', Name: '', Type: '' }} />)
        component.unmount()
    })

    it('Should handle on action menu click', (done: MochaDone) => {
        const component = renderer.create(<ActionsCell actions={[]} openActionMenu={() => {
            done()
            component.unmount()
