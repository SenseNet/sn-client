import { IODataParams, ODataFieldParameter } from "../Models/IODataParams";
...
export class ODataUrlBuilder {
     ...
     */
    public static readonly ODATA_PARAMS = ["select", "expand", "orderby", "top", "skip", "filter", "format", "inlinecount"];
     ...
     */
    public static buildUrlParamString<T>(config: RepositoryConfiguration, options?: IODataParams<T>): string {
        if (!options) {
            options = {};
        }
        ...
        if (config.requiredSelect === "all" || config.defaultSelect === "all" || options.select === "all") {
            ...
            options.select = this.combineODataFieldParameters<any>(config.requiredSelect, options.select || config.defaultSelect) as any;
        }
