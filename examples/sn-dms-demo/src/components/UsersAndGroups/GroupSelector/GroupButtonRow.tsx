...
export class GroupButtonRow extends Component<GroupButtonRowProps> {
    ...
    public render() {
        return (<div style={styles.buttonContainer as any}>
            <Button color="default" style={{ marginRight: 20, color: '#fff' }} onClick={() => this.handleCancelClick()}>{resources.CANCEL}</Button>
            <Button onClick={() => this.handleSubmitClick()} variant="raised" color="secondary">{resources.ADD}</Button>
        </div>)
    }
}
